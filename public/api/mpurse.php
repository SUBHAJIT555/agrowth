...
function siteUrl()
{
    return rtrim(envVal('SITE_URL', 'https://agentmart-ai.com'), '/');
}
...
{
    $fullName = trim(v('name', v('billing_name')));
    ...
    $address = v('billing_address', v('address', v('company')));
    $town = v('billing_town', v('town', v('city')));
    $state = v('billing_state', v('state'));
    $postcode = v('billing_postcode', v('postcode'));
    $notes = v('notes', v('message'));

    if ($firstName === '') {
        fail(422, 'Your name is required.');
    }
    ...
    if ($address === '' || $town === '') {
        fail(422, 'Street address and city are required.');
    }

    list($items, $total) = parseCart();
    $amount = number_format($total, 2, '.', '');
    $orderId = 'AMA' . strtoupper(bin2hex(random_bytes(8)));
    ...
    if ($description === '') {
        $description = 'Agent Mart AI order';
    }
...
function applyGatewayUpdate($orderId, $gateway, $lookupMiss)
{
    ...
    if ($order && !$lookupMiss) {
        ...
        if ($status === 'success') {
            ...
            $needMerchant = empty($order['emailed_merchant']);
            $needCustomer = empty($order['emailed_customer']);
            if ($needMerchant || $needCustomer) {
                $sent = sendOrderPaidEmail($order, $gateway);
                $order['emailed_merchant'] = !empty($sent['merchant']) || !empty($order['emailed_merchant']);
                $order['emailed_customer'] = !empty($sent['customer']) || !empty($order['emailed_customer']);
                $order['emailed'] = !empty($order['emailed_merchant']) || !empty($order['emailed_customer']);
                if (empty($order['emailed_merchant']) || empty($order['emailed_customer'])) {
                    $err = function_exists('ne_mail_last_error') ? ne_mail_last_error() : '';
                    $order['mail_error'] = $err !== '' ? $err : 'Could not send confirmation email.';
                } else {
    ...
    return array(
        ...
        'receipt_sent' => $order && (!empty($order['emailed_merchant']) || !empty($order['emailed_customer'])),
        'mail_error' => $order && !empty($order['mail_error']) ? (string) $order['mail_error'] : '',
    );
}
...
{
    $brandName = envVal('MAIL_FROM_NAME', 'Agent Mart AI');
...
{
    $brandName = envVal('MAIL_FROM_NAME', 'Agent Mart AI');
    $toEmail = envVal('ORDER_NOTIFY_EMAIL', 'info@agentmart-ai.com');
    $border = '#e5e7eb';
    $brandColor = '#e04300';
    ...
    $subject = 'Paid order - Agent Mart AI - ' . $orderId;
    ...
    $sentCustomer = !empty($order['emailed_customer']);
    if (!$sentCustomer && $email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $customerIntro = '<tr><td style="padding:0 24px 8px;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#333;">'
            . '<p>Hi ' . clean($name) . ',</p>'
            . '<p>We have received your payment of <strong>INR ' . clean($amount) . '</strong> for order <strong>' . clean($orderId) . '</strong>. Our team will activate your service shortly.</p>'
            . '</td></tr>';
        $sentCustomer = deliverMail(
            $email,
            $name,
            'Payment received - ' . $brandName . ' - ' . $orderId,
            wrapEmail('Payment received', $customerIntro . $mainContent, $email),
            'Payment received for order ' . $orderId . "\nAmount: INR " . $amount . "\nTxn: " . $txnId . "\nRRN: " . $rrn
        );
    }

    return array(
        'merchant' => $sentMerchant,
...
function sendBarePaidEmail($orderId, $gateway)
{
    ...
    $main = '<tr><td style="padding:0 24px 24px;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#333;">'
        ...
        . kvRow('Txn ID', $txnId)
        . '</td></tr>';
    deliverMail($toEmail, 'Agent Mart AI', $subject, wrapEmail($subject, $main, $toEmail), $subject);
}
