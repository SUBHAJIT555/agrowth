<?php
/**
 * Agent Mart AI mailer — contact, newsletter, quote, partnership forms.
 */
...
$allowed = array(
    'https://agentmart-ai.com',
    'https://www.agentmart-ai.com',
    'http://agentmart-ai.com',
    'http://www.agentmart-ai.com',
    'http://localhost:3000',
    'http://127.0.0.1:3000',
);
...
{
    global $inputData;
    $raw = $inputData['services'] ?? ($inputData['service'] ?? ($inputData['service[]'] ?? ''));
    if (is_array($raw)) {
...
{
    $brandName = ne_env('MAIL_FROM_NAME', 'Agent Mart AI');
    $domain = ne_env('SITE_URL', 'https://agentmart-ai.com');
...
<body style="margin:0;padding:0;background:#fff6ee;">
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:30px 10px;">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:100%;background:#ffffff;border:1px solid ' . $border . ';border-radius:8px;overflow:hidden;">
      <tr><td align="center" style="padding:24px 16px;background:linear-gradient(135deg,#ff8a3d,' . $accent . ');">
        <h1 style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:22px;color:#ffffff;">' . clean($brandName) . '</h1>
        <p style="margin:8px 0 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#fff4ea;">' . clean($tagline) . '</p>
      </td></tr>
      ...
      <tr><td align="center" style="padding:0 20px 16px;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#6b7280;">Received at ' . date('Y-m-d H:i:s') . ' IST</td></tr>
      ' . $mainContent . '
      <tr><td align="center" style="padding:14px 20px;background:#f3f4f6;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#6b7280;">
        Sent from the <strong>' . clean($brandName) . '</strong> website (' . clean($domain) . ') to ' . clean($toEmail) . '.
      </td></tr>
    </table>
  </td></tr></table>
</body></html>';
...
if (!in_array($formType, $allowedTypes, true)) {
    json_exit(400, array('success' => false, 'error' => 'Invalid formType.', 'message' => 'Invalid formType.'));
}

if ($formType === 'contact') {
    $nameCheck = firstFilled(array('name', 'your-name', 'fullName', 'firstName'));
    $emailCheck = firstFilled(array('email', 'billing_email'));
    $msg = $nameCheck === '' ? 'Name is required' : ($emailCheck === '' ? 'Email is required' : null);
} elseif ($formType === 'newsletter') {
    $msg = requiredFields(array('email' => 'Email'));
} elseif ($formType === 'partnership') {
    $nameCheck = firstFilled(array('fullName', 'name', 'your-name'));
    $emailCheck = firstFilled(array('email'));
    $companyCheck = firstFilled(array('companyName', 'company'));
    if ($nameCheck === '') {
...
if ($msg) {
    json_exit(422, array('success' => false, 'error' => $msg, 'message' => $msg));
}

$email = firstFilled(array('email', 'billing_email', 'shipping_email'));
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    json_exit(422, array('success' => false, 'error' => 'Invalid email.', 'message' => 'Invalid email.'));
}

$name = firstFilled(array('name', 'your-name', 'fullName', 'billing_first_name', 'firstName'));
if ($formType === 'quote') {
    $name = trim(v('billing_first_name') . ' ' . v('billing_last_name'));
}
if ($formType === 'partnership') {
    $name = firstFilled(array('fullName', 'name', 'your-name'));
}

$brandName = ne_env('MAIL_FROM_NAME', 'Agent Mart AI');
$toEmail = ne_env('MAIL_TO', ne_env('ORDER_NOTIFY_EMAIL', 'info@agentmart-ai.com'));

if ($formType === 'contact') {
    $subject = 'New contact inquiry — ' . $brandName . ' — ' . $name;
} elseif ($formType === 'newsletter') {
    $subject = 'Newsletter signup — ' . $brandName . ' — ' . $email;
} elseif ($formType === 'partnership') {
    $subject = 'Partner application — ' . $brandName . ' — ' . $name;
} else {
    $subject = 'Quote request — ' . $brandName . ' — ' . $name;
}
...
if ($formType === 'contact') {
    $fullName = firstFilled(array('name', 'your-name', 'fullName'));
    $mainContent =
        '<tr><td style="padding:0 24px 24px;"><div style="padding:12px;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#0b0314;">'
        . kvRow('Name', $fullName)
        . kvRow('Email', $email)
        . kvRow('Phone', v('phone'))
        . kvRow('Company', firstFilled(array('company', 'companyName')))
        . kvRow('Contact type', v('contactType', v('contact-type')))
        . kvRow('Contact details', v('contactDetails', v('contact-details')))
        . kvRow('Services', $services)
    ...
    $mainContent =
        ...
        . kvRow('Name', $name)
        . kvRow('Email', $email)
        . kvRow('Company', firstFilled(array('companyName', 'company')))
        ...
        . kvRow('Message', v('message'), true)
        . '</div></td></tr>';
    $alt .= 'Partner: ' . $name . ' — ' . firstFilled(array('companyName', 'company')) . "\n";
} else {
    $cartHtml = '';
    $cart = cartItems();
    if (count($cart)) {
...
$sent = ne_deliver_mail($toEmail, $brandName, $subject, $html, $alt, $email, $name);

if ($sent && in_array($formType, array('contact', 'partnership'), true)) {
    $ackSubject = 'We received your message — ' . $brandName;
    $ackHtml = '<p style="font-family:Arial,sans-serif;font-size:14px;color:#0b0314;">Hi ' . clean($name) . ',</p>'
        . '<p style="font-family:Arial,sans-serif;font-size:14px;color:#0b0314;">Thank you for contacting <strong>' . clean($brandName) . '</strong>. We have received your '
        . ($formType === 'partnership' ? 'partner application' : 'message')
        . ' and will get back to you shortly.</p>'
        . '<p style="font-family:Arial,sans-serif;font-size:14px;color:#0b0314;">Regards,<br><strong>' . clean($brandName) . ' Team</strong></p>';
    ne_deliver_mail($email, $name, $ackSubject, $ackHtml, strip_tags($ackHtml), '', '');
}

if (!$sent) {
    $err = function_exists('ne_mail_last_error') && ne_mail_last_error() !== ''
        ? ne_mail_last_error()
